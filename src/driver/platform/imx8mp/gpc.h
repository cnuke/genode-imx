...
struct Gpc
{
	...
	Domain pcie            { powers, env.pd(), "pcie",             1 };
	Domain usb1            { powers, env.pd(), "usb1",             2 };
	Domain usb2            { powers, env.pd(), "usb2",             3 };
	Domain mlmix           { powers, env.pd(), "mlmix",            4 };
	Domain audiomix        { powers, env.pd(), "audiomix",         5 };
	Domain gpumix          { powers, env.pd(), "gpumix",           6 };
	Domain gpu2d           { powers, env.pd(), "gpu2d",            7 };
	Domain gpu3d           { powers, env.pd(), "gpu3d",            8 };
	...
	Domain mipi1           { powers, env.pd(), "mipi1",           15 };
	Domain mipi2           { powers, env.pd(), "mipi2",           16 };
	Domain hdmimix         { powers, env.pd(), "hdmimix",         17 };
	Domain hdmi            { powers, env.pd(), "hdmi",            18 };
